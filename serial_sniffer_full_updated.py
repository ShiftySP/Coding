...
class SerialSnifferApp:
    ...
    def create_widgets(self):
        ...
        self.combo_a = ttk.Combobox(frm_top,
                                    textvariable=self.port_a,
                                    width=10)
        self.combo_a.grid(row=0, column=1, padx=5)
        ttk.Checkbutton(
            frm_top,
            text="Enable Port A",
            variable=self.enable_port_a
        ).grid(row=1, column=1, sticky="w")

        # Port B
        ttk.Label(frm_top, text="Port B:").grid(row=0, column=2, sticky="w")
        self.combo_b = ttk.Combobox(frm_top,
                                    textvariable=self.port_b,
                                    width=10)
        self.combo_b.grid(row=0, column=3, padx=5)
        ttk.Checkbutton(
            frm_top,
            text="Enable Port B",
            variable=self.enable_port_b
        ...
        # Data bits
        ttk.Label(frm_top, text="Data Bits:")\
        ...
        # Stop bits
        ttk.Label(frm_top, text="Stop Bits:")\
        ...
        # Buttons (Connect / Disconnect / Pause / Resume / Export / Clear Display / Dark Mode)
        frm_btns = ttk.Frame(self.master)
        frm_btns.pack(fill="x", padx=10, pady=5)

        self.btn_connect = ttk.Button(
            frm_btns, text="Connect", command=self.connect_serial
        )
        ...
        self.btn_pause = ttk.Button(
            frm_btns, text="Pause Text", command=self.toggle_pause, state="disabled"
        )
        ...
        self.btn_export = ttk.Button(
            frm_btns, text="Export Log", command=self.export_log
        )
        self.btn_clear_disp = ttk.Button(
            frm_btns, text="Clear Text", command=self.clear_display
        )
        ttk.Checkbutton(
            frm_btns,
            text="Dark Mode",
        ...
        self.text_display = tk.Text(
            self.master, wrap="none", state="disabled", height=20
        )
        self.text_display.pack(
            expand=True, fill="both", padx=10, pady=(2, 5)
        )
        ...
        # Byte counters
        frm_cnt = ttk.Labelframe(self.master, text="Bytes Counter")
        frm_cnt.pack(fill="x", padx=10, pady=(0, 10))
        self.label_a_to_b = ttk.Label(
            frm_cnt, text="A → B Bytes: 0", font=("Arial", 13)
        )
        self.label_b_to_a = ttk.Label(
            frm_cnt, text="B → A Bytes: 0", font=("Arial", 13)
        )
        self.label_total = ttk.Label(
            frm_cnt, text="Total Bytes: 0", font=("Arial", 13)
        )
        ttk.Button(
            frm_cnt,
            text="Reset Counter",
        ...
        # Send command
        frm_send = ttk.Labelframe(self.master, text="Send Command")
        frm_send.pack(fill="x", padx=10, pady=5)
        ttk.Label(frm_send, text="Command:")\
            .grid(row=0, column=0, sticky="sw", padx=0, pady=0)
        ...
        self.btn_send_it.grid(
            row=0, column=5, sticky="sw", padx=(6), pady=0
        )
        ...
        self.btn_clear = ttk.Button(
            frm_send,
            text="Clear",
            command=lambda: self.entry_command.delete(0, tk.END)
        )
        self.btn_clear.grid(row=0, column=6, padx=0, pady=0)

        # Entry for command
        self.entry_command = tk.Entry(frm_send, width=40)
        self.entry_command.grid(
            row=0, column=1, sticky="sw", padx=0, pady=0
        )
        self.entry_command.bind(
            "<Return>",
            lambda e: (self.send_command(), "break")
        )

        ttk.Label(frm_send, text="")\
            .grid(row=0, column=2, sticky="nw", padx=0, pady=0)
        ttk.Combobox(
            frm_send,
            textvariable=self.send_port,
            values=("A", "B"),
            width=5
        ).grid(row=0, column=2, sticky="sw", padx=6, pady=0)

        ttk.Label(frm_send, text="Interval (s):")\
            .grid(row=3, column=1, sticky="w", padx=5, pady=5)
        ttk.Entry(
            frm_send,
            textvariable=self.auto_send_interval,
            width=10
        ).grid(row=3, column=1, sticky="w", padx=50, pady=25)

        self.btn_auto_send = ttk.Button(
            frm_send, text="Start Auto-Send", command=self.toggle_auto_send
        )
